...
export async function doctorCommand() {
    ...
    console.log('');
    // Repo wiring
    check('graph.json', existsSync(join(repoRoot, 'graph.json')), existsSync(join(repoRoot, 'graph.json')) ? 'found' : 'missing — run turbo-flow init');
    check('specs/', existsSync(join(repoRoot, 'specs')), existsSync(join(repoRoot, 'specs')) ? 'found' : 'missing');
    check('prompts/', existsSync(join(repoRoot, 'prompts')), existsSync(join(repoRoot, 'prompts')) ? 'found' : 'missing');
    check('gates/', existsSync(join(repoRoot, 'gates')), existsSync(join(repoRoot, 'gates')) ? 'found' : 'missing');
    ...
    if (existsSync(join(repoRoot, 'graph.json'))) {
        try {
            ...
            check('graph edges', errors.length === 0, errors.length === 0 ? 'valid' : errors.join('; '));
        }
        catch (e) {
