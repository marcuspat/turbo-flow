...
import { createInitialState, readState, writeState, setActive } from '../schemas/state.js';
...
export async function runEngine(opts) {
    ...
    try {
        ...
        return { exitCode: EXIT.ERROR, state: null, message: `failed to load graph.json: ${err}` };
    }
    const errors = validateGraph(graph);
    if (errors.length > 0) {
        return { exitCode: EXIT.ERROR, state: null, message: `graph validation failed:\n${errors.join('\n')}` };
    }
    ...
    if (!state) {
        if (opts.resume) {
            return { exitCode: EXIT.ERROR, state: null, message: `no run state found for ${specId}` };
        }
        const specPath = join(repoRoot, 'specs', `${specId}.md`);
        if (!existsSync(specPath)) {
            return { exitCode: EXIT.ERROR, state: null, message: `spec not found: specs/${specId}.md` };
        }
    ...
    while (state.status === 'running') {
        ...
        if (state.node_iteration >= maxIter) {
            ...
            // Escalate
            const escResult = await escalate(state, `Node "${state.graph_node}" failed ${maxIter} iterations without passing gates. Options: (1) continue with current output, (2) abort the run, (3) adjust the spec and retry.`, { repoRoot, mention: graph.escalation?.mention, channel: graph.escalation?.channel });
        ...
        if (nodeResult.error) {
            ...
            const escResult = await escalate(state, `Node "${state.graph_node}" encountered an error: ${nodeResult.error}. Options: (1) retry, (2) skip this node, (3) abort the run.`, { repoRoot, mention: graph.escalation?.mention, channel: graph.escalation?.channel });
        ...
        if (nodeResult.gatesPassed === null) {
            ...
            const escResult = await escalate(state, nodeResult.humanQuestion ?? 'A decision is needed.', { repoRoot, mention: graph.escalation?.mention, channel: graph.escalation?.channel });
            ...
            if (state.node_iteration >= (node.max_iterations ?? 3)) {
                const escResult = await escalate(state, `Node "${state.graph_node}" failed gates after ${state.node_iteration} iterations.\n\n${nodeResult.gateFeedback}`, { repoRoot, mention: graph.escalation?.mention, channel: graph.escalation?.channel });
...
async function executeNode(node, state, host, opts) {
...
function advanceNode(state, node, passed) {
