...
import { validateVerdict } from '../schemas/verdict.js';
/** Run all gates for a node. Cmd gates first, then judge gates. */
export async function runGates(node, lastOutput, host, opts) {
    ...
    if (!anyFailed) {
        for (const gate of judgeGates) {
            const start = Date.now();
            const result = await runJudgeGate(gate, lastOutput, host, opts);
            results.push(result);
            if (!result.passed) {
                anyFailed = true;
...
export function runCmdGate(gate, repoRoot, specId) {
    const start = Date.now();
    const cmd = (gate.run ?? '')
...
async function runJudgeGate(gate, content, host, opts) {
    const start = Date.now();
